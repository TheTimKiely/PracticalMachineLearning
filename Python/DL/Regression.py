...
from PracticalMLCore.ModelLayer import ModelLayer
...
if(mode == 't'):
    ...
    prediction_space = np.linspace(min(norm_test_data[:, 5]), max(norm_test_data[:, 5])).reshape(-1, 1)
    plt.plot(prediction, norm_test_data[:, 5], color='blue')
    plt.show()
print('done')
